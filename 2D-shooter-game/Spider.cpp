...
void Spider::doIt(Player& player)
{
	...
	{
		move(Vector2f(0, SPIDER_SPEED));
	}
	if (player.getY() + player.getSizeY() / 2 < sprite.getPosition().y)
	{
		move(Vector2f(0, -SPIDER_SPEED));
	}
	if (player.getX() - sprite.getPosition().x <= player.getSizeX() / 2 && player.getX() - sprite.getPosition().x >= -
		player.getSizeX() / 2)
	{
		if (player.getY() - sprite.getPosition().y <= player.getSizeY() / 2 && player.getY() - sprite.getPosition().y >=
			-player.getSizeY() / 2)
		{
			if (attacked)
			{
...
int Spider::checkColision(vector<Bullet> bulletsList)
{
	...
	{
		if (bulletsList[i].getX() >= sprite.getPosition().x - texture.getSize().x / 2 && bulletsList[i].getX() <= sprite
		                                                                                                          .getPosition()
		                                                                                                          .x +
			texture.getSize().x / 2)
		{
			if (bulletsList[i].getY() >= sprite.getPosition().y - texture.getSize().y / 2 && bulletsList[i].getY() <=
